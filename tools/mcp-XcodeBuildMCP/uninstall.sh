#!/usr/bin/env bash
# XcodeBuildMCP — uninstall ONE MCP server, into a given repo.
#
# Headless Xcode builds — no Xcode approval prompt.
#
...
#
# Dependencies: node_npx, claude, npm:xcodebuildmcp
#
# Usage: ./uninstall.sh --into <repo>
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BUNDLE="$HERE/../mcp-bundle"
exec "$BUNDLE/uninstall.sh" "$@" XcodeBuildMCP
