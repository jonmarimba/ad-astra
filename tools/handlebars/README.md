...
*"I can ride my bike with no handlebars."* wrap-in-app's design has a property beyond "fix the mail wrapper". The 2026-08-12 design has the `.app` reference the script by absolute path, so the script never lives inside the bundle. Granting a `.app` bundle a macOS TCC permission grants *that code-signed identity*, not *a task*. The app's hash — and therefore its grant — survives any edit to the script it calls. So one wrapper granted **every** TCC permission it will ever plausibly need becomes a standing tower. Mint it once, reprogram what it does forever by editing `handlebars.sh`, never touch the app, never re-grant.
...
- `Handlebars.app` — the granted identity. **Never edit this directly**. A wrap-in-app re-run over it, or any change to the bundle itself, changes its hash and silently kills every grant it holds. The dead grant resurfaces as EPERM with no prompt — exactly the original TCC disease.
...
Jonathan, 2026-08-14: *"we do one at a time in the script. And coordinate where I grant access in the seat with your direction."* He followed immediately with the point of that constraint. Grant everything at once and *"then you have a skeleton key to the system."* This is the actual design, not "grant every pane up front":
...
Run `handlebars.sh` with no argument to see current state across all 8 wired domains (Full Disk Access, Screen Recording, Automation/Notes, Contacts, Calendar, Accessibility, Microphone, Camera). The run is read-only: it checks only what's already been granted and never prompts for anything new. Each BLOCKED line prints the exact System Settings pane to visit. Mic and Camera probes need ffmpeg; without it they report SKIP instead of BLOCKED.
...
- Never let a scheduled/unattended run carry a destructive or irreversible action without the same explicit-permission discipline that applies everywhere else in this repo. That discipline covers sending messages, deleting data, and spending money — see the outer system prompt's action categories.
...
The HOA/ingest wrappers are narrowly granted (FDA only, one job each). That is the deliberately safer default, and it is still the right call for anything that doesn't need this breadth. Handlebars exists for the case where you genuinely want one standing, broadly-capable identity you reprogram on the fly. An example is the interactive one-off: "I need Automation + Screen Recording for ten minutes today, something else tomorrow." The alternative is minting a new narrowly-scoped wrapper — and re-granting FDA — every time the need shifts.
