...
Turn video(s) into a timestamped HTML contact sheet. Then you can scan an hour of footage in a minute and jump to the moments that matter. Built for storm-drainage clips reviewed against a punchlist, but it's generic — any video, any reason.
...
The test runs real ffmpeg on generated test footage. It asserts the frame count matches the requested rate, the timestamps are right, and every frame is referenced in the sheet. The RED cases (missing video, non-video bytes, bad flags) all fail as they should.
