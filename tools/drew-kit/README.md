...
Wire Drew's shared agents-and-prompts components (Swift style rules, Jira conventions, etc.) into a *specific* repo's `CLAUDE.md`/`AGENTS.md` as a marked block of **repo-relative** `@`-imports. Never install them as a copy over the global file (see `WARNING-drews-update-sh.md` for why Drew's own `update.sh` must not run here).
...
The test asserts by effect. The block lands in both files, and imports are repo-relative with **no** absolute/`/Users/` path leaking. The copied component is present and non-empty, refresh doesn't duplicate, and `--set` switching works. Uninstall removes the block + files while preserving pre-existing prose. It runs a **real** `subtree` install against a throwaway local git source repo, which proves the method itself rather than only its guard. It RED-controls the method guards (unknown method, submodule/subtree without `--src`), an unknown set, broken markers, and a missing repo arg.
