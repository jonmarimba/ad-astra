...
assert_blocked "verb=kill; exec \$verb $victim" 'exec wrapped variable-expanded command is unsupported'
assert_blocked "verb=kill; exec \"-a\" harmless \$verb $victim" 'quoted exec -a wrapped variable command is unsupported'
assert_blocked "x=i; k\${x}ll $victim" 'assignment-spliced variable command is unsupported'
continued=$'k\\\nill '
