...
- The combined daemon's `tools/list` returns correctly prefixed names for both upstreams, and a prefixed call routes to the right one — `xcode__XcodeListWindows` returns a real workspace path from mcpbridge, `drews__version` returns Drew's real version string, verified independently. The `xcode__`/mcpbridge side only works while the screen is unlocked — see the third gotcha below; `drews__` has no such dependency.
...
**mcpbridge needs the screen unlocked, full stop — this is external to the daemon, not a bug in it.** Confirmed live, 2026-08-14: with the console locked (`CGSSessionScreenIsLocked=Yes`), every real `xcode__` call fails almost instantly with `Connection closed`, even on a connection that had already connected and initialized cleanly. A brand-new, never-approved connecting PID behaves differently but is just as stuck — its very first real request (even `tools/list`) hangs indefinitely with no dialog ever appearing anywhere in the Accessibility tree, because Xcode can't render or process the Allow prompt while the console is locked. `check-allow-window.sh` correctly reports no pending dialog in both cases — there genuinely isn't one to click; the OS won't show it while locked, so `_click_allow_if_present()` has nothing to act on and the daemon's own auto-approve logic isn't the fix here. This traced through a chain of separate-looking symptoms that were all this one cause: `_run_osascript` timeouts against a wedged System Events, orphaned `osascript` processes, and finally the immediate `Connection closed` on every real mcpbridge call — verify with `python3 -c "import subprocess; print(subprocess.run(['ioreg','-n','Root','-d1'],capture_output=True,text=True).stdout)" | grep -i locked` (or just `ioreg -n Root -d1 | grep -i "CGSSessionScreenIsLocked\|IOConsoleLocked"`) before chasing anything else in this daemon when `xcode__` calls start failing. `drews__` calls are unaffected — Drew's tool never touches the GUI.
