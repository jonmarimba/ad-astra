...
AI-legible text sidecars for PDFs, generated automatically on commit. A PDF is opaque to text tooling (grep, agents, graph ingestion). This kit writes, next to each `foo.pdf`, the text layers an agent can actually read:
...
Designed to be a **flat git submodule** of any repo. One source of truth: update the submodule and every repo's hook behavior updates with it. The hook resolves the kit through `git config jsutils.path`, so there's nothing to re-splice.
...
`setup.sh` is idempotent and **non-clobbering** — it only manages its own block in `.git/hooks/pre-commit`; any other hook content is preserved.
...
Marked 3 (3.1.21, Setapp) ships an AppleScript dictionary with `convert_to` / `fetch_profile_names` / export profiles — on paper the native headless export. In practice the sdef's command codes are malformed (4–6 chars where AppleEvents require 8: `conv`, `gpls`, `mkopst`), so terminology never loads. AppleScript throws -2753 and JXA throws -1708, by bundle id or path, app running or not. Until Brett fixes the dictionary, **md2pdf (pandoc→weasyprint + the attorney templates) is the headless legal-export path**. `marked_to_pdf.sh` (UI-scripting Marked 2's print dialog) remains a deprecated fallback. Worth reporting upstream — support@marked2app.com / @ttscoff.

## Where this copy came from, and the question that is not settled

This directory is a SNAPSHOT of `git@github.com:jonmarimba/pdf-sidecars.git`, vendored into astra on 2026-08-12. The upstream repository's last commit is 2026-08-10, so nothing has diverged yet. As of 2026-08-18 all three copies on this machine are byte-identical: this one, the standalone checkout at `~/svnCheckouts/pdf-sidecars`, and the older set in `js-utils/pdf`. Its own commit message calls itself "the canonical PDF sidecar toolkit".
...
- If the GitHub project is still the home, astra should pull from it on install rather than carrying a frozen copy. In that case, `js-utils/pdf` should be retired.
