...
#      claude -p (Claude), codex exec (GPT), qwen -p (Qwen/omniroute). Group review, not per-tool,
#      so we don't multiply inference for little gain: each brand is told to be thorough across ALL
#      the listed tools in one pass.
#
# Usage:
#   convene.sh --name <cluster> --repo <dir> --files "f1 f2 ..." \
#              --convoq "term1|term2|..." [--qwen-model <id>] [--timeout <secs>] [--out <dir>]
#
...
NAME=""; REPO=""; FILES=""; CONVOQ=""; QMODEL="${QWEN_MODEL:-}"; TIMEOUT=300; OUT=""
...
while [ $# -gt 0 ]; do case "$1" in
  ...
  --convoq) CONVOQ="$2"; shift 2;; --qwen-model) QMODEL="$2"; shift 2;;
  --timeout) TIMEOUT="$2"; shift 2;; --out) OUT="$2"; shift 2;;
  *) echo "unknown arg '$1'" >&2; exit 64;;
esac; done
[ -n "$NAME" ] && [ -n "$REPO" ] && [ -n "$FILES" ] || { echo "usage: convene.sh --name X --repo D --files '...' [--convoq 'a|b'] [--qwen-model id]" >&2; exit 64; }
...
( cd "$REPO" && run_brand qwen   qwen ${QMODEL:+-m "$QMODEL"} -p "$P" ) &
wait
