#!/usr/bin/env bash
# test-convoke.sh — convoke fans one task file out to each requested agent binary, captures
...
CONVOKE="$HERE/../convocation/convoke"
...
export CLAUDE_BIN="$SB/bin/fake-claude" CODEX_BIN="$SB/bin/fake-codex" QWEN_BIN="$SB/bin/fake-claude"
assert_rc 0 "fan-out to three agents succeeds" "$CONVOKE" "$SB/task.md" --out "$SB/out" --agents claude,codex,qwen --tag r1
...
CLAUDE_BIN="$SB/bin/noisy" "$CONVOKE" "$SB/task.md" --out "$SB/out2" --agents claude --tag r1 >/dev/null 2>&1
...
red "requested agent with missing binary must fail (no silent skip)" env CLAUDE_BIN="$SB/bin/does-not-exist" "$CONVOKE" "$SB/task.md" --out "$SB/out3" --agents claude --tag r1
red "unknown agent name must fail" "$CONVOKE" "$SB/task.md" --out "$SB/out4" --agents gemini --tag r1
