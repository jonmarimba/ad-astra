---
name: ios-ui-driving
description: Drive iOS app UI in the simulator the Maharam way. That means accessibility identifiers everywhere, AX-tree targeting for ad-hoc probes (axe or ios-simulator MCP), and promotion of stable flows to XCUITest-by-identifier. Use when tapping, typing, reading, or verifying iOS UI.
---
...
The **probe tier** is the default for exploration and one-offs. Cheap, no code written. Read the tree, act by identifier:
...
Promote a flow to the **regression tier** when it earns it. When you are going to work on a feature for a while, an XCUITest that drives its flow by identifier is super valuable. You rerun the exact same sequence after every change instead of re-tapping it by hand. It runs in-sim so it never grabs the host mouse, and when the feature ships the test stays in the repo as a regression. The regression tier pairs with the probe tier rather than replacing it. The XCUITest carries the stable spine of the flow. The axe/ios-simulator probes handle the ad-hoc pokes around it (checking a value mid-flow, trying a variant, exploring an unexpected state). Writing XCUITest is time-consuming - pay that cost only when it is the fast path NET. That is true when you will iterate on this feature across many builds. It is also true when re-tapping the flow by hand is what is taking forever, or when the flow must survive as a regression guard.
